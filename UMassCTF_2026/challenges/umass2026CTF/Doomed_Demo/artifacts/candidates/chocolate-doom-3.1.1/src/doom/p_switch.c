...
static void LogSwitchUse(line_t *line)
{
	FILE *stream;
	...
	stream = fopen("/tmp/doom_usepos.log", "a");
	...
	fprintf(
		stream,
		"USEPOS: line=%d special=%d ang=0x%x x=0x%x y=0x%x\n",
		(int) (line - lines),
		line->special,
		(unsigned int) usething->angle,
		(unsigned int) usething->x,
		(unsigned int) usething->y
	);
	fclose(stream);
}
