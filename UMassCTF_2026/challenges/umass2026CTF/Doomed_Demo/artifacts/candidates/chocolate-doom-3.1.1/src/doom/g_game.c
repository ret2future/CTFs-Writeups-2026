...
static void MaybeApplyDebugSpawnOverride(void)
{
    const char *x_env;
    const char *y_env;
    const char *angle_env;
    mobj_t *mo;
    fixed_t x;
    fixed_t y;
    unsigned int angle;

    x_env = getenv("CHOC_POS_X");
    y_env = getenv("CHOC_POS_Y");

    if (x_env == NULL || y_env == NULL)
    {
        return;
    }
    ...
    x = (fixed_t) strtol(x_env, NULL, 0);
    y = (fixed_t) strtol(y_env, NULL, 0);
    angle_env = getenv("CHOC_POS_ANGLE");
    angle = angle_env == NULL ? mo->angle : (unsigned int) strtoul(angle_env, NULL, 0);

    if (!P_TeleportMove(mo, x, y))
    {
        printf("DEBUGSPAWN: failed x=0x%x y=0x%x\n", (unsigned int) x, (unsigned int) y);
        return;
    }
    ...
    printf("DEBUGSPAWN: ang=0x%x x=0x%x y=0x%x\n", angle, (unsigned int) x, (unsigned int) y);
}

static int GetDebugEnvInt(const char *name, int default_value)
{
    const char *value;
...
static void MaybeApplyDebugTiccmdOverride(ticcmd_t *cmd)
{
    int forward_tics;
    int use_tics;

    forward_tics = GetDebugEnvInt("CHOC_AUTO_FORWARD_TICS", 0);
    use_tics = GetDebugEnvInt("CHOC_AUTO_USE_TICS", 0);

    if (leveltime < forward_tics)
    {
        cmd->forwardmove = forwardmove[1];
    }

    if (leveltime < use_tics)
    {
        cmd->buttons |= BT_USE;
    }
}
...
{ 
    ...
    MaybeApplyDebugTiccmdOverride(cmd);
...
{ 
    ...
    {
        fprintf(stderr,
                "FINALPOS: ang=0x%x x=0x%x y=0x%x\n",
                (unsigned int) players[consoleplayer].mo->angle,
                (unsigned int) players[consoleplayer].mo->x,
                (unsigned int) players[consoleplayer].mo->y);
    }
