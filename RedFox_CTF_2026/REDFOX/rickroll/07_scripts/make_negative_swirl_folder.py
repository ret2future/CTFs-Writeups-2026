...
def main() -> None:
    ...
    src = root / "image.png"
    out_dir = root / "negative_swirl_image_png"
    out_dir.mkdir(parents=True, exist_ok=True)

    img = Image.open(src).convert("L")
    arr = np.array(img)

    intensities = [0] + [x for x in range(-20, -501, -20)]
    for intensity in intensities:
        swirled = reverse_swirl(arr, float(intensity))
        out_path = out_dir / f"negative_swirl_{intensity:+04d}.png"
        Image.fromarray(swirled).save(out_path)

    print(f"Source: {src}")
    print(f"Output folder: {out_dir}")
    print(f"Generated files: {len(intensities)}")
